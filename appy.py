...
def count_winners():
    # Supabaseから当選者数を取得
    response = supabase.table('user').select('result').eq('result', '当たり').execute()
...
def register():
    if request.method == 'POST':
        ...
        try:
            # Supabaseに新しいユーザーを登録
            supabase.table('user').insert({'username': username, 'password': password, 'result': ''}).execute()
...
def enter_lottery():
    ...
    # 結果をSupabaseに保存
    supabase.table('user').update({'result': result}).eq('username', username).execute()
